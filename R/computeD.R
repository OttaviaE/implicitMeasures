...
#'  \item{\code{dscore_dX}}{The average \emph{D-score} for the pratice and test
...
computeD <- function(data,
  ...
  # create the variable for the order of presentation of the conditions
  condition_order <- aggregate(condition ~ participant, data = data,
                               unique)
  ...
  # filter fast participants (more than 10% of trials < 300 ms)
  ...
  if (dim(table_300)[1] == 1){
    ...
    # merge with the descript data
    descript_data <- merge(descript_data, nfast300,
                           by ="participant")
    # compute the proportion
  ...
  sbj_data_wide$dscore <- with(sbj_data_wide,
                               (rowSums(
                                 sbj_data_wide[,c("d_practice",
                                                  "d_test")]))/2)
  sbj_data_wide <- sbj_data_wide[, c("participant", "mean_practice_MappingA",
                                     "mean_test_MappingA", "mean_practice_MappingB",
                                     "mean_test_MappingB", "d_practice", "d_test",
                                     "dscore")]
