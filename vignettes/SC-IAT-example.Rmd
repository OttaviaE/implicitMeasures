...
The labels for identifying the responses beyond the rtw (and that hence have to be eliminated) must be included in the variable identified by `trial_id`, but they have to be specified via the `non_response` in function `Dsciat()` function to actually be deleted.
...
Function `Dsciat()` results in a `data.frame` with class `dsciat` containing a number of rows equal to the number of participants, their *D-score*, and a bunch of useful information on their performance (see the documentation for the `Dsciat()` function for further information on the resulting data frame). Functions `dscr_d()`, `d_plot()`, and `d_distr()` require the object resulting from function `Dsciat()` to work. 

### Descriptive statistics

The descriptive statistics of the *D-score* and of the response times in the two critical conditions can be easily obtained by using function `descript_d()`: 
...
By specifying argument `latex = TRUE`, function `dscr_d()` prints the results in a LaTeX compatible format: 
...
#### Indivdual respondent level 

`d_plot()` plots the SC-IAT *D-score* for each respondent. 


```{r scpointDefault, fig.align='center', fig.width=6, fig.cap="\\label{fig:scpointDefault} Default use of function d_plot() function"}
 d_plot(d_sciat1) # Data frame containing SC-IAT D-scores
```

Clearly, in case of large sample size, as in this case, the label identifying each respondent is not easy to read, and it can be elimnated by setting the argument `x_values` to `FALSE`. 
...
Function `d_distr()` plots the distribution of the SC-IAT *D-score*s. It provides different options for choosing the most appropriate representation. Figure \ref{fig:scsampleDeafult} represents the default output of `d_distr()`.


```{r scsampleDefault, fig.align='center', fig.width=6, fig.cap="\\label{fig:scsampleDefault} Default use of d_plot() function"}
...
```{r fig.align='center', fig.width=6, fig.cap="\\label{fig:multiscDefault} Default results represention of function multi_dsciat()"}
...
The labels identifying each SC-IAT can be set with arguments `label_sc1` and `label_sc2` (default are `SC-IAT 1` and `SC-IAT 2`). The values labels of the x-axis can be removed by setting `x-values = FALSE` (suggested in case of big sample size).
...
Figure \ref{fig:multiscSettings} depicts an example of the graphical representation of the results obtained by changing the settings in function `multi_dsciat()`.


```{r fig.align='center', fig.width=6, fig.cap="\\label{fig:multiscSettings} Results representation of function multi_dsciat() with settings change"}
multi_dsciat(d_sciat1, # dataframe containing the results of the first SC-IAT
             d_sciat2, # dataframe containing the results of the second SC-IAT
